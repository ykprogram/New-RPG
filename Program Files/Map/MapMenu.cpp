...
StringScript_t MenuString[3]={
	...
	{20,480-125+20+20,GetColor(255,0,255),"しかし特に変わったところはないようだ。"},
	//{20,480-125+20+20+20,GetColor(255,255,255),"表示出力中"}
};
...
void Search(){
	MessageFrame();
	//もし文字がすべて表示し終えたら戻る
	if( DrawMessageString2( 0, 1, MenuString) == 1 )MapMenu[1].Flag=0;
}
...
void StatusMenu( Character_t *Character ){
	...
	for(int i=0;i<2;i++){
		FontNumDraw(120+60*i,38,(Character+i)->LEVEL);
		FontNumDraw(120+60*i,69,(Character+i)->HP);
		FontNumDraw(120+60*i,99,(Character+i)->MP);
		FontNumDraw(120+60*i,129,(Character+i)->Power);
		FontNumDraw(120+60*i,162,(Character+i)->Defense);
		FontNumDraw(120+60*i,195,(Character+i)->Money);
		FontNumDraw(120+60*i,223,(Character+i)->Speed);
	}
}