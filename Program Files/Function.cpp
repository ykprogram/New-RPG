...
int DrawMenu(int ElementMaxY, int ElementMaxX, int KeyCode, MenuElement_t Menu[], int action ){
	// 計算フェーズ
	...
	if( CheckHitKey2( KEY_INPUT_RIGHT ) == 1 ){PlaySE(select2);SelectNum2 = (SelectNum2 + 1)% ElementMaxX ;}				// 現在の選択項目を一つ右にずらす(ループ)
	if( CheckHitKey2( KEY_INPUT_LEFT ) == 1 ){PlaySE(select2);SelectNum2 = (SelectNum2 + ElementMaxX - 1)%ElementMaxX;}	// 現在の選択項目を一つ左にずらす(逆ループ)
...
char test[3][100]={{""},{""},{""}};		//三行の透明の枠があると考えると良い
// 会話などを表示する,文は三行まで　文字は全角が好ましい
//(半角、全角の順で並んでいて半角の数が奇数の場合全角が表示させるとき一瞬点になることがあるので)
int DrawMessageString2(int Start, int End, StringScript_t MenuString[]){
	...
	for( int i=Start; i<=(NowNumber+Start); i++){
		DrawFormatString(MenuString[i].x,MenuString[i].y,MenuString[i].color,"%s",test[i]);
